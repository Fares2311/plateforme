        ...
        <div style={{ maxWidth: '900px', margin: '0 auto', padding: '2rem' }}>
            ...
            <div className="card card-glass text-center mb-8 fade-enter" style={{ position: 'relative', overflow: 'hidden', padding: '4rem 2rem' }}>
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                    {Array.isArray(project.category) ? (
                        ...
                        <div className="badge" style={{ background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.2)', color: '#fff' }}>
                            <Hash size={14} style={{ marginRight: '4px' }} /> {project.category}
                        </div>
                )}
                </div>
                <h1 className="mb-4 text-gradient" style={{ fontSize: '3rem' }}>{project.title}</h1>
                ...
                <div className="flex justify-center gap-6 mb-8 flex-wrap">
                    ...
                    <div className="stat-box" style={{ width: '130px' }}>
                        <div className="stat-value">{project.target_hours}h</div>
                        <div className="stat-label">
                            {project.goal_frequency === 'daily' ? '/ jour' : project.goal_frequency === 'weekly' ? '/ semaine' : project.goal_frequency === 'monthly' ? '/ mois' : 'Target'}
                        </div>
                    </div>
                    <div className="stat-box" style={{ width: '130px' }}>
            ...
            <nav className="tabs-nav mt-8">
                ...
                <button className={`tab-btn ${activeTab === 'activity' ? 'active' : ''}`} onClick={() => setActiveTab('activity')}><TrendingUp size={16} /> Activité</button>
            </nav>
            ...
            <div className="tabs-content relative">
                ...
                {activeTab === 'overview' && (
                    <motion.div
                        ...
                        className="tab-pane active" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }} >
                        ...
                        {project.description && (
                            <div className="card card-glass fade-enter" style={{ borderLeft: '3px solid var(--color-primary)' }}>
                                ...
                                <p style={{ margin: 0, lineHeight: 1.7, opacity: 0.85 }}>{project.description}</p>
                            </div>
                )}
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: '1rem' }}>
                            ...
                            <div className="card card-glass text-center" style={{ padding: '1rem' }}>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {Array.isArray(project.category) ? (
                                        ...
                                        <div style={{ fontWeight: 700, fontSize: '0.9rem' }}>{project.category}</div>
                )}
                                </div>
                            ...
                            <div className="card card-glass text-center" style={{ padding: '1rem' }}>
                                <div style={{ fontSize: '1.8rem', marginBottom: '0.25rem' }}>⏱</div>
                                <div style={{ fontWeight: 700, fontSize: '0.9rem' }}>{project.target_hours}h {project.goal_frequency === 'daily' ? '/ jour' : project.goal_frequency === 'weekly' ? '/ semaine' : project.goal_frequency === 'monthly' ? '/ mois' : ''}</div>
                            ...
                            {milestones.length > 0 && (
                                <div className="card card-glass text-center" style={{ padding: '1rem' }}>
                                    ...
                                    <div style={{ fontSize: '0.75rem', opacity: 0.5, marginTop: '2px' }}>Projet groupe</div>
                                </div>
                )}
                            {/* Upcoming sessions */}
                            {sessions.length > 0 && (
                    ...
                    <motion.div
                        ...
                        <div className="card card-glass flex flex-col" style={{ height: '540px' }}>
                            ...
                            {(project?.pinned_messages || []).length > 0 && (() => {
                                const pinned = (project.pinned_messages as string[])
                                                ...
                                                {isSystem ? (
                                                    ...
                                                    <>
                                                        ...
                                                        <div style={{ maxWidth: '75%', position: 'relative' }}>
                                                            ...
                                                            {/* Reply preview */}
                                                            {msg.reply_to && (
                                                                <div style={{ padding: '4px 8px', marginBottom: '4px', borderLeft: '3px solid var(--color-primary)', background: 'rgba(99,102,241,0.08)', borderRadius: '6px', fontSize: '0.75rem', color: 'var(--color-text-secondary)', cursor: 'default' }}>
                                                                    <span style={{ fontWeight: 700, color: 'var(--color-primary)' }}>{msg.reply_to.user_name} </span>
                                                                    <span style={{ opacity: 0.8 }}>{msg.reply_to.content.slice(0, 80)}{msg.reply_to.content.length > 80 ? '...' : ''}</span>
                                                                </div>
                                                            )}
                                                            <div className={`chat-bubble ${isMe ? 'bg-primary text-white' : ''}`}>{renderMessageContent(msg.content)}</div>
                                                        ...
                                                        {hoverMsgId === msg.id && (
                                                            ...
                                                            >
                                                                ...
                                                                <button
                                                                    onClick={() => { setReplyingTo({ id: msg.id, user_name: msg.user_name, content: msg.content }); chatInputRef.current?.focus(); }}
                                                                    style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '2px 6px', borderRadius: '6px', color: 'rgba(255,255,255,0.5)', fontSize: '11px', fontWeight: 600, marginLeft: '2px', display: 'flex', alignItems: 'center', gap: '3px' }}
                                                                    title="Répondre"
                                                                >
                                                                    ↩ Rép.
                                                                </button>
                            ...
                            {/* Reply banner */}
                            {replyingTo && (
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 16px', background: 'rgba(99,102,241,0.08)', borderLeft: '3px solid var(--color-primary)', margin: '0 1rem 6px', borderRadius: '6px', fontSize: '0.8rem' }}>
                                    <span style={{ color: 'var(--color-text-secondary)' }}>
                                        Réponse à <span style={{ fontWeight: 700, color: 'var(--color-primary)' }}>{replyingTo.user_name}</span> — {replyingTo.content.slice(0, 60)}{replyingTo.content.length > 60 ? '...' : ''}
                                    </span>
                                    <button onClick={() => setReplyingTo(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-text-tertiary)', padding: '2px', display: 'flex' }}><X size={14} /></button>
                                </div>
                            )}
                            {/* @mention dropdown */}
                            {mentionQuery !== null && (
                                <div style={{ margin: '0 1rem 6px', background: 'var(--color-bg-surface-elevated)', border: '1px solid var(--color-border)', borderRadius: '10px', overflow: 'hidden', boxShadow: '0 4px 16px rgba(0,0,0,0.3)' }}>
                                    {memberships.filter((m: any) => m.full_name?.toLowerCase().includes(mentionQuery)).slice(0, 5).map((m: any) => (
                                        <button key={m.user_id} type="button" onClick={() => insertMention(m.full_name)} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: '10px', padding: '8px 14px', background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left', transition: 'background 0.15s' }} onMouseEnter={e => (e.currentTarget.style.background = 'rgba(255,255,255,0.05)')} onMouseLeave={e => (e.currentTarget.style.background = 'none')}>
                                            <img src={m.photo_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(m.full_name)}&background=random`} alt={m.full_name} style={{ width: '24px', height: '24px', borderRadius: '50%', objectFit: 'cover' }} />
                                            <span style={{ fontSize: '0.875rem', fontWeight: 600 }}>{m.full_name}</span>
                                        </button>
                                    ))}
                                    {memberships.filter((m: any) => m.full_name?.toLowerCase().includes(mentionQuery)).length === 0 && (
                                        <div style={{ padding: '8px 14px', fontSize: '0.8rem', color: 'var(--color-text-tertiary)' }}>Aucun membre trouvé</div>
                                    )}
                                </div>
                            )}
                            <form onSubmit={handleSendMessage} className="flex gap-2 p-4 pt-0 mt-auto">
                                <input ref={chatInputRef} type="text" value={newMessage} onChange={handleChatInputChange} className="input flex-grow" placeholder={t('room_chat_placeholder')} required autoComplete="off" />
                            ...
                            <div className="flex-grow overflow-y-auto mb-4 px-4 custom-scrollbar" id="ai-chat-messages" style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                {aiMessages.length === 0 ? (
                                    <div className="text-center text-secondary my-auto p-4 flex flex-col items-center">
                                        ...
                                        <p>{t('room_ai_coach_empty').replace('{}', project.title)}</p>
                                    </div>
                                ) : (
                ...
                {activeTab === 'milestones' && (
                    <motion.div
                        ...
                        <ProjectBoard projectId={id as string} user={user} projectMembers={memberships} onActivity={logActivity} />
                    </motion.div>
                )}
                ...
                {activeTab === 'activity' && (
                    <motion.div
                        ...
                        className="tab-pane active">
                        <div className="flex justify-between items-center mb-6" style={{ borderBottom: '1px solid var(--color-border)', paddingBottom: '1rem' }}>
                            <h3 className="flex items-center gap-2 m-0"><TrendingUp className="text-primary" /> Fil d'activité</h3>
